...
//echo $r_date;
$daily_query="SELECT SERVICE_NAME,SERVICE_TYPE,OPERATOR,SHORTCODE,NEW_SUBS,TOTAL_SUBS,FREE_DOWNS,CHARGED_DOWNS,TOTAL_DOWNS,RDATE,SUBS_REV,SUBS_CHARGED,SUBS_REV,TOTAL_REV,WAP_PUSHED FROM REPORTS_WAP WHERE trunc(RDATE)=TO_DATE('{$r_date}','MM-DD-YYYY') ORDER BY trunc(RDATE)";
$result = helper_exec_select_query($conn, $daily_query);
$sql_query = "SELECT SERVICE_NAME,SUM(NEW_SUBS) as NSubscriber,SUM(FREE_DOWNS) as FDowns,SUM(CHARGED_DOWNS) as ChargedDowns,SUM(TOTAL_DOWNS) as TotalDowns,SUM(SUBS_CHARGED) as SubsCharged,SUM(SUBS_REV) as SubsRev,SUM(TOTAL_REV) as TotalRev,SUM(WAP_PUSHED) as WapPushed FROM REPORTS_WAP WHERE trunc(RDATE)=TO_DATE('{$r_date}','MM-DD-YYYY') GROUP BY SERVICE_NAME ORDER BY SERVICE_NAME";
//echo '....>'.$daily_query;
