...
if (( $_REQUEST['fdate'] != '') && ($_REQUEST['sdate'] != '')) {
    ...
    $sql_query   = "SELECT * FROM REPORTS_SMS_VASPRO WHERE RDATE between to_date('{$final_date}', 'MM-DD-YYYY') and to_date('$nextday', 'MM-DD-YYYY')" . $extra_sql . $extra_sql2 . $extra_sql3;
    //echo $sql_query;
}
else {
    if ($_REQUEST['sdate'] != '') {
        ...
        $sql_query  = "SELECT * FROM REPORTS_SMS_VASPRO WHERE trunc(RDATE) = TO_DATE('{$final_date}','MM-DD-YYYY')" . $extra_sql . $extra_sql2 . $extra_sql3;
        // echo $sql_query;
    }
    else {
        $dateShow2  = $_REQUEST['fdate'];
