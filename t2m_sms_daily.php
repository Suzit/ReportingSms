...
//echo $r_date;
$daily_query="SELECT SERVICE_NAME,SERVICE_TYPE,OPERATOR,SHORTCODE,NEW_SUBS,TOTAL_SUBS,NEW_CHURN,TOTAL_CHURN,TOTAL_BASE,RDATE,KEYWORD,MO,MT FROM REPORTS_SMS WHERE trunc(RDATE)=TO_DATE('{$r_date}','MM-DD-YYYY')";
$result = helper_exec_select_query($conn, $daily_query);
$sql_query = "SELECT SERVICE_NAME,SUM(NEW_SUBS) as NSubscriber,SUM(NEW_CHURN) as NChurn,SUM(TOTAL_SUBS) as SumSubscriber,SUM(MO) as Mob,SUM(MT) as MsgTermination FROM REPORTS_SMS WHERE trunc(RDATE)=TO_DATE('{$r_date}','MM-DD-YYYY') GROUP BY SERVICE_NAME ORDER BY SERVICE_NAME";
//echo '....>'.$sql_query;
...
<div id="content">
    ...
    <br><br/>
    <?php
    $sql_query = "SELECT OPERATOR,SUM(NEW_SUBS) as Total_Sub,SUM(NEW_CHURN) as Total_Churn,SUM(TOTAL_SUBS) as SumSubscriber,SUM(MO) as Mob,SUM(MT) as MsgTermination FROM REPORTS_SMS WHERE trunc(RDATE)=TO_DATE('{$r_date}','MM-DD-YYYY') GROUP BY OPERATOR ORDER BY OPERATOR";
    $res       = helper_exec_select_query($conn, $sql_query);
    /*
      echo '<pre>';
      print_r($res);
